	...
	case WM_INITDIALOG:
		{
			...
			const char* pRenderFmt = "Renderer:\t%s";
			const char* pVersionFmt = "Version:\t\t%s";
			const char* pVendorFmt = "Vendor:\t\t%s";
			const char* pExtensionFmt = "\n%s";

			const char* pGLrenderer = glGetString( GL_RENDERER );
			const char* pGLversion = glGetString( GL_VERSION );
			const char* pGLvendor = glGetString( GL_VENDOR );
			const char* pGLextensions = glGetString( GL_EXTENSIONS );

			extensions = malloc( (size_t)snprintf( NULL, 0, pExtensionFmt, pGLextensions ) );

			sprintf( renderer, pRenderFmt, pGLrenderer );
			sprintf( version, pVersionFmt, pGLversion );
			sprintf( vendor, pVendorFmt, pGLvendor );
			sprintf( extensions, pExtensionFmt, pGLextensions );
