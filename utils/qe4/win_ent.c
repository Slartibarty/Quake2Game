    ...
    LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		if (LOWORD(wParam) == VK_TAB)
		{
			...
			{
				SendMessage (hwndEnt[EntValueField], WM_SETTEXT, 0, (LPARAM)"");
...
BOOL CreateEntityWindow(HINSTANCE hInstance)
{
	...
	OldFieldWindowProc = (void *)GetWindowLongPtr (hwndEnt[EntKeyField], GWLP_WNDPROC);
	SetWindowLongPtr (hwndEnt[EntKeyField], GWLP_WNDPROC, (LONG_PTR)FieldWndProc);
	SetWindowLongPtr (hwndEnt[EntValueField], GWLP_WNDPROC, (LONG_PTR)FieldWndProc);

	OldEntityListWindowProc = (void *)GetWindowLongPtr (hwndEnt[EntList], GWLP_WNDPROC);
	SetWindowLongPtr (hwndEnt[EntList], GWLP_WNDPROC, (LONG_PTR)EntityListWndProc);
