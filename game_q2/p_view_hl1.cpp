...
float V_CalcBob(edict_t* ent)
{
	...
	bobtime += FRAMETIME;
	cycle = bobtime - (int)(bobtime / 0.8f) * 0.8f;
	cycle /= 0.8f;

	if (cycle < 0.5f)
	{
		cycle = M_PI_F * cycle / 0.5f;
	}
	else
	{
		cycle = M_PI_F + M_PI_F * (cycle - 0.5f) / (1.0 - 0.5f);
	}
	...
	bob = sqrt(vel[0] * vel[0] + vel[1] * vel[1]) * 0.01f;
...
#else
	ent->client->ps.gunangles[YAW] = ent->client->ps.viewangles[YAW] + 0;
	ent->client->ps.gunangles[PITCH] = -ent->client->ps.viewangles[PITCH] + 0 * 0.25;
	ent->client->ps.gunangles[ROLL] -= sin(level.time * 0.5) * 0.1;

	// don't apply all of the v_ipitch to prevent normally unseen parts of viewmodel from coming into view.
	ent->client->ps.gunangles[PITCH] -= sin(level.time * 1) * (0.3 * 0.5);
	ent->client->ps.gunangles[YAW] -= sin(level.time * 2) * 0.3;
#endif
}
