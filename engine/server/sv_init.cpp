...
static void SV_SpawnServer (char *server, char *spawnpoint, serverState_t serverstate, qboolean attractloop, qboolean loadgame)
{
	...
	{
	//	sv.models[1] = CM_LoadMap (nullptr, false, &checksum);	// no real map
...
void SV_InitGame (void)
{
	...
	svs.spawncount = rand();
	svs.clients = (client_t*)Mem_ClearedAlloc (sizeof(client_t)*maxclients->value); // Assume we want calloc for this
	svs.num_client_entities = maxclients->value*UPDATE_BACKUP*64;
	svs.client_entities = (entity_state_t*)Mem_ClearedAlloc (sizeof(entity_state_t)*svs.num_client_entities); // Assume we want calloc for this
	...
	NET_StringToNetadr (idmaster, master_adr[0]);
