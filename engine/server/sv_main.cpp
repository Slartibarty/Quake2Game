...
static void SVC_Info()
{
	...
	else
	{
		...
		Q_sprintf_s( string, "%16s %8s %2i/%2i\n", hostname->GetString(), sv.name, count, maxclients->GetInt() );
	}
...
static void SVC_DirectConnect()
{
	...
	if ( sv.attractloop )
	{
		...
		{
			Com_Print( "Remote connect in attract loop.  Ignored.\n" );
			Netchan_OutOfBandPrint( NS_SERVER, adr, "print\nConnection refused.\n" );
			return;
		}
	}
	...
	for ( i = 0, cl = svs.clients; i < maxclients->GetInt(); i++, cl++ )
	{
		...
		{
			if ( !NET_IsLocalAddress( adr ) && ( svs.realtime - cl->lastconnect ) < ( SEC2MS( sv_reconnect_limit->GetInt() ) ) )
			{
	...
	{
		Netchan_OutOfBandPrint( NS_SERVER, adr, "print\nServer is full.\n" );
		Com_DPrint( "Rejected a connection.\n" );
		return;
	}
...
gotnewcl:
	...
	*newcl = temp;
	...
	if ( !ge->ClientConnect( ent, userinfo ) )
	{
		...
		Com_DPrint( "Game rejected a connection.\n" );
		return;
	}
...
{
	if ( strlen( rcon_password->GetString() ) == 0 ) {
		return 0;
	}
...
static void SV_ConnectionlessPacket()
{
	...
	if ( Q_strcmp( c, "getchallenge" ) == 0 ) {
		SVC_GetChallenge();
		return;
	}
	if ( Q_strcmp( c, "connect" ) == 0 ) {
		SVC_DirectConnect();
		return;
	}
	if ( Q_strcmp( c, "rcon" ) == 0 ) {
		SVC_RemoteCommand();
		return;
	}
...
static void SV_CheckTimeouts()
{
	...
	droppoint = svs.realtime - 1000 * timeout->GetInt();
	zombiepoint = svs.realtime - 1000 * zombietime->GetInt();
...
static void SV_RunGameFrame()
{
	...
	// don't run if paused
	if ( !sv_paused->GetBool() || maxclients->GetInt() > 1 )
	{
		ge->RunFrame();
...
void Master_Heartbeat()
{
	...
	// pgm post3.19 change, cvar pointer not validated before dereferencing
	if ( !public_server || !public_server->GetBool() ) {
...
void SV_Init()
{
	...
	Cvar_Get( "protocol", STRINGIFY( PROTOCOL_VERSION ), CVAR_SERVERINFO | CVAR_NOSET );;
