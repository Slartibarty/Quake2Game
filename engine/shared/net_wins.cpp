...
void NetadrToSockadr (netadr_t *a, sockaddr *s)
{
	...
	{
		((sockaddr_ipx *)s)->sa_family = AF_IPX;
		memcpy(((sockaddr_ipx *)s)->sa_netnum, &a->ipx[0], 4);
		memcpy(((sockaddr_ipx *)s)->sa_nodenum, &a->ipx[4], 6);
		((sockaddr_ipx *)s)->sa_socket = a->port;
	}
	else if (a->type == NA_BROADCAST_IPX)
	{
		((sockaddr_ipx *)s)->sa_family = AF_IPX;
		memset(((sockaddr_ipx *)s)->sa_netnum, 0, 4);
		memset(((sockaddr_ipx *)s)->sa_nodenum, 0xff, 6);
		((sockaddr_ipx *)s)->sa_socket = a->port;
	}
}
...
char	*NET_AdrToString (netadr_t a)
{
	...
	if (a.type == NA_LOOPBACK)
		...
		Com_sprintf (s, "%02x%02x%02x%02x:%02x%02x%02x%02x%02x%02x:%i", a.ipx[0], a.ipx[1], a.ipx[2], a.ipx[3], a.ipx[4], a.ipx[5], a.ipx[6], a.ipx[7], a.ipx[8], a.ipx[9], ntohs(a.port));
...
qboolean	NET_StringToSockaddr (char *s, sockaddr *sadr)
{
	hostent	*h;
	char	*colon;
	int		val;
	...
	else
	{
		...
		for (colon = copy ; *colon ; colon++)
			...
			{
				*colon = 0;
				((sockaddr_in *)sadr)->sin_port = htons((short)atoi(colon+1));	
			}
		
		if (copy[0] >= '0' && copy[0] <= '9')
		{
			*(int *)&((sockaddr_in *)sadr)->sin_addr = inet_addr(copy);
		}
		else
		{
