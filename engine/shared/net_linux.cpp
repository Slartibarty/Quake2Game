//=================================================================================================
// Linux sockets implementation of Quake's networking layer
//=================================================================================================
...
static void NET_NetadrToSockadr( const netadr_t *a, sockaddr_in *s )
{
	...
	case NA_BROADCAST:
		...
		break;
	case NA_IP:
		s->sin_addr.s_addr = (uint32_t)a->ip.ui;
		break;
	}
}
...
static bool NET_StringToSockaddr( const char *s, sockaddr_in *sadr )
{
	...
	for ( char *colon = copy; *colon; ++colon )
	{
		...
		{
			*colon = '\0';
			sadr->sin_port = htons( (uint16_t)atoi( colon + 1 ) );
		}
	}
...
void NET_Init()
{
	Com_Print( "Linux sockets Initialized\n" );
