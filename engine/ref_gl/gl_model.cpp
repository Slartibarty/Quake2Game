...
mleaf_t *Mod_PointInLeaf (vec3_t p, model_t *model)
{
	...
	if (!model || !model->nodes)
		RI_Com_Error (ERR_DROP, "Mod_PointInLeaf: bad model");
...
model_t *Mod_ForName (const char *name, qboolean crash)
{
	...
	if (!name[0])
		RI_Com_Error (ERR_DROP, "Mod_ForName: NULL name");
	...
	if (name[0] == '*')
	{
		...
		if (i < 1 || !r_worldmodel || i >= r_worldmodel->numsubmodels)
			RI_Com_Error (ERR_DROP, "bad inline model number");
		return &mod_inline[i];
	}
	...
	if (i == mod_numknown)
	{
		if (mod_numknown == MAX_MOD_KNOWN)
			RI_Com_Error (ERR_DROP, "mod_numknown == MAX_MOD_KNOWN");
		mod_numknown++;
	}
	...
	{
		if (crash)
			RI_Com_Error (ERR_DROP, "Mod_NumForName: %s not found", mod->name);
	...
	default:
		RI_Com_Error (ERR_DROP,"Mod_NumForName: unknown fileid for %s", mod->name);
		break;
	}
...
void Mod_LoadVertexes (lump_t *l)
{
	...
	if (l->filelen % sizeof(*in))
		RI_Com_Error (ERR_DROP, "MOD_LoadBmodel: funny lump size in %s",loadmodel->name);
...
//			RI_Com_Error (ERR_DROP, "Bad surface extents");
	}
}
...
void Mod_LoadFaces (lump_t *l)
{
	...
	for ( surfnum=0 ; surfnum<count ; surfnum++, in++, out++)
	{
		...
		if (ti < 0 || ti >= loadmodel->numtexinfo)
			RI_Com_Error (ERR_DROP, "MOD_LoadBmodel: bad texinfo number");
...
{	
	...
	for ( i=0 ; i<count ; i++)
	{
		...
		if (j < 0 ||  j >= loadmodel->numsurfaces)
			RI_Com_Error (ERR_DROP, "Mod_ParseMarksurfaces: bad surface number");
...
{	
	...
	if (count < 1 || count >= MAX_MAP_SURFEDGES)
		RI_Com_Error (ERR_DROP, "MOD_LoadBmodel: bad surfedges count in %s: %i",
...
void Mod_LoadBrushModel (model_t *mod, void *buffer)
{
	...
	if (loadmodel != mod_known)
		RI_Com_Error (ERR_DROP, "Loaded a brush model after the world");

	header = (dheader_t *)buffer;

	i = LittleLong (header->version);
	if (i != BSPVERSION)
		RI_Com_Error (ERR_DROP, "Mod_LoadBrushModel: %s has wrong version number (%i should be %i)", mod->name, i, BSPVERSION);
	...
	for (i=0 ; i<mod->numsubmodels ; i++)
	{
		...
		*starmod = *loadmodel;
		...
		if (starmod->firstnode >= loadmodel->numnodes)
			RI_Com_Error (ERR_DROP, "Inline model %i has bad firstnode", i);
...
void Mod_LoadAliasModel (model_t *mod, void *buffer)
{
	...
	if (version != ALIAS_VERSION)
		RI_Com_Error (ERR_DROP, "%s has wrong version number (%i should be %i)",
...
//		RI_Com_Error (ERR_DROP, "model %s has a skin taller than %d", mod->name,
//				   MAX_LBM_HEIGHT);

	if (pheader->num_xyz <= 0)
		RI_Com_Error (ERR_DROP, "model %s has no vertices", mod->name);

	if (pheader->num_xyz > MAX_VERTS)
		RI_Com_Error (ERR_DROP, "model %s has too many vertices", mod->name);

	if (pheader->num_st <= 0)
		RI_Com_Error (ERR_DROP, "model %s has no st vertices", mod->name);

	if (pheader->num_tris <= 0)
		RI_Com_Error (ERR_DROP, "model %s has no triangles", mod->name);

	if (pheader->num_frames <= 0)
		RI_Com_Error (ERR_DROP, "model %s has no frames", mod->name);
...
void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
	...
	if (sprout->numframes > MAX_MD2SKINS)
		RI_Com_Error (ERR_DROP, "%s has too many frames (%i > %i)",
